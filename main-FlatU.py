...
def compute_reward(proposer):
    ...
    for node in allNodes: 
        ...
        if node.nodeId == proposer:
            reward_basic = B_i*rate # don't forget rate
            node.w += reward_basic
    return transfers
...
def main():
    ...
    results.to_csv('data/chap4/flat_n1k_r10k.csv', index=False)
    # print(results.head())
    # data = results.groupby(["round","parent"]).agg({'round': 'first', 'nodeId': 'first','w': 'sum', 'W': 'first', 'proposer': 'first'})
    # data.to_csv('data/LongRuns/flat_%10_n1k_r10k_grouped.csv', index=False)
